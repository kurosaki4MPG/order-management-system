...
- 実機確認では、請求先名の `fontWeight` を太字から通常に戻すことで、先頭文字が欠ける現象が解消した
- このため、PDF ライブラリ本体の汎用不具合というより、`@react-pdf/renderer` と日本語フォントの太字描画経路の相性問題として扱うのが妥当だった
- 裏付けとして、`@react-pdf/renderer` の公式 Fonts ドキュメントでは `fontWeight` ごとのフォント登録と最寄り重みへのフォールバックが案内されており、公式 issue でも日本語フォントや `fontWeight: 700` に関する事例が報告されている
- 太字描画はカスタム日本語フォントとの相性差が出るため、`fontWeight` を変えて切り分けるのが有効だった
