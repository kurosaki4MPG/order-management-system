...
- 太字描画はカスタム日本語フォントとの相性差が出るため、`fontWeight` を変えて切り分けること
...
- 環境変数は「設定値」であって、秘密情報と同じ箱に入れないほうがよいこと
...
- 認証だけでは不十分で、変更系 API には同一オリジン確認も必要なこと
...
- ログは文字列よりも 1 行 JSON の構造化ログにしたほうが、後から検索しやすいこと
- `requestId` / `eventId` / `orderId` / `userId` を同じキーで扱うと、API と AWS 側を横断して追跡しやすいこと
...
- 障害対応は認証、API、非同期処理、帳票生成の 4 つに分けて切り分けること
- まず CloudWatch Alarm で対象サービスを特定し、次に CloudWatch Logs で `requestId` / `eventId` / `orderId` を追うこと
- Step Functions の `Executions` と SQS / DLQ を合わせて見ると、非同期処理の止まり方を追いやすいこと
...
- フロント、API、AWS を 1 つの業務システムとして統合して確認すること
- 注文登録から請求書生成、保存、署名付き URL、通知までを通しで確認すること
- 認証、監視、非同期処理が途切れずにつながることを確認すること
...
- 性能、可用性、保守性、運用性、セキュリティの観点を分けて確認すること
...
- 命名、責務分離、例外処理、再利用性、テストをレビュー観点として整理すること
- 直すべき箇所と残す理由を分けて記録すること
- 実運用で困りそうな箇所を最後に見直すこと
...
- README、設計書、手順書、実装、テストを第三者向けに整理すること
- 成果物の役割と所在を明確にすること
- 学習案件として完了した状態にすること
