...
- 単体、統合、E2E を分けてテスト対象を整理し、画面表示・API 契約・Lambda・永続化の境界をそれぞれ確認できるようにした
- `case-summary.md` では相対パスのテストファイル名と、どのケースが pass / fail したかを追いやすい形に整えた
- `test:coverage` は品質基準として使う一方で、100% に届かない箇所は重要度に応じて理由を明示する運用にした
